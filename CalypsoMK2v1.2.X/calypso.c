...
#define MassageDelay    500         // sonst 500
...
void Calypso_Init(void){                                                // "Calypso_Init"
    ...
    //while(UART1_IsRxReady() == false);                                  // warte solange "UART1_IsRxReady" gleich "false" ist
...
void Calypso_MQTT_write(char MQTT_Topic[80], char MQTT_payload[30]){    // "Calypso_MQTT_write"
    uint8_t MQTT_payload_length = 0;                                    // definiert "MQTT_payload_length"
    
    MQTT_payload_length = strlen(MQTT_payload);                         // schreibt die Länge von "MQTT_payload" in "MQTT_payload_length"
    sprintf(AT_Data, "AT+MqttPublish=0,%s,QoS0,0,%d,%s\r\n",            // schreibt "AT+MqttPublish=0,%s,QoS0,0,%d,%s\r\n" in "AT_Data"
            MQTT_Topic,                                                 // 
            MQTT_payload_length,                                        // 
            MQTT_payload);                                              // 
    Calypso_AT_Send();                                                  // ruft "Calypso_AT_Send" auf
    while(UART1_IsRxReady() == false);                                  // warte solange "UART1_IsRxReady" gleich "false" ist
    MQTT_Massage_delay();                                               // ruft "MQTT_Massage_delay" auf
}                                                                       // 
...
void MQTT_Massage_delay(void){                                          // "MQTT_Massage_delay"
    __delay_ms(MassageDelay);                                           // wartet "MassageDelay" ms
}                                                                       // 